...
# 设置处理过的GTEx文件夹路径
folder_path = r'C:\Users\87485\OneDrive - Florida State University\LLM\rawdata\gtexdata\GTEx_Analysis_v8_eQTL'

# 过滤出以 v8.sgenes.txt.gz 结尾的文件
files = [f for f in os.listdir(folder_path) if f.endswith('v8.egenes.txt.gz')]
    ...
    for _, row in group.iterrows():
        ...
        if row['region'].startswith('Brain'):
            ...
            output_list.append({
                ...
                'Region': row['region'],
                'Source': 'eQTL'  # 添加 source 列，内容为 'eQTL'
            })

# 将结果保存为CSV文件
output_csv_file = r'C:\Users\87485\OneDrive - Florida State University\LLM\preprocessed data\eqtl.csv'
